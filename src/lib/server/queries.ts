...
export async function loadPublicPlate(filename: string): Promise<{
  ...
  if (path.startsWith("https://")) {
    try {
      ...
      const folder = kind === "thumbnail" ? "thumbs" : "previews";
      const match = pathname.match(new RegExp(`^/${folder}/(.+)-([a-f0-9]{8,})\\.(?:jpe?g|png|webp)import { getSql } from "@/lib/db";
import { parseDeviceType, deviceWhere, type DeviceFilter } from "@/lib/device";
import { asCardThumb, isLiveFormat, resolveThumb, resolvePreview } from "@/lib/media";
import { wallpaperAlt } from "@/lib/seo";
import { daySeed, mergeHomeDuos, parseDuoSlug, suggestCompanion, suggestDuos } from "@/lib/duos";
...
, "i"));
      if (match) mediaId = `${match[1]}-${kind === "thumbnail" ? "thumb" : "prev"}-${match[2]}`;
    } catch {
      // Non-standard external URLs can still use the original redirect behavior below.
    }
    if (!mediaId) return { redirect: path };
  }
  const id = mediaId ?? (path.startsWith("/api/media/") ? path.slice("/api/media/".length) : path);
